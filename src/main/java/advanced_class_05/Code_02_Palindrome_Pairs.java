...
public class Code_02_Palindrome_Pairs {
	...
	public static List<List<Integer>> findAll(String word, int index,
		...
		Integer rest = set.get(""); // 在set中把空字符串的位置拿出来，不是null！是空字符串
		if (rest != null && rest != index && word.equals(reverse)) {
			addRecord(res, rest, index);
			addRecord(res, index, rest);
		}
		int[] rs = manacherss(word);// 生成回文半径数组
		int mid = rs.length >> 1;
		for (int i = 1; i < mid; i++) {
			if (i - rs[i] == -1) {
				rest = set.get(reverse.substring(0, mid - i)); // O(L)
		...
		for (int i = mid + 1; i < rs.length; i++) {
			if (i + rs[i] == rs.length) {
				rest = set.get(reverse.substring((mid << 1) - i));
				if (rest != null && rest != index) {
					addRecord(res, index, rest);
				}
			}
