...
public class IndexMinHeap<Item extends Comparable> {
    ...
    private void swapIndexes(int i, int j){
        int t = indexes[i];//堆中i的位置，对应的索引为t
        indexes[i] = indexes[j];
        indexes[j] = t;
        reverse[indexes[i]] = i;//索引t，对应的堆中的位置。
        reverse[indexes[j]] = j;
    }
    //********************
    //* 最小索引堆核心辅助函数
    ...
    public static void main(String[] args) {
        ...
        /*for( int i = 0 ; i < N ; i ++ )
            indexMinHeap.insert( i ,i );
        indexMinHeap.getMinIndex();*/
        IndexMinHeap<Integer> indexMinHeap = new IndexMinHeap<Integer>(5);
        indexMinHeap.insert( 0 ,2 );
        indexMinHeap.insert( 1 ,1 );
        indexMinHeap.insert( 2 ,4 );
        indexMinHeap.insert( 3 ,9 );
        indexMinHeap.insert( 4 ,6 );
        System.out.println(indexMinHeap.getItem( 0 ));
    }
}