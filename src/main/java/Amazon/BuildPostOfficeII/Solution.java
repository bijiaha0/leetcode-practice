package Amazon.BuildPostOfficeII;
...
public class Solution {
    ...
    public int shortestDistance(int[][] grid) {
        ...
        List<Coordinate> houses = getCoordinates(HOUSE);//houses
        int[][] distanceSum = new int[n][m];
        int[][] visitedTimes = new int[n][m];
        for (Coordinate house : houses) {//从房屋出发BFS
            bfs(house, distanceSum, visitedTimes);
        }

        int shortest = Integer.MAX_VALUE;
        List<Coordinate> empties = getCoordinates(EMPTY);//empty
