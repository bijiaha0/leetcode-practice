...
但是如果， 先把长度60的金条分成30和30，花费60 再把长度30金条分成10和20，花费30 一共花费90铜板。
 输入一个数组，返回分割的最小代价。
——————————————————————————
 每次从小跟堆里弹出两个，然后求和，放回去，然后再弹出两个，求和再放回去，依次(哈弗曼编码)。
* */
public class Code_02_Less_Money {
	public static int lessMoney(int[] arr) {
		PriorityQueue<Integer> pQ = new PriorityQueue<>();//默认是小跟堆
		for (int i = 0; i < arr.length; i++) {//将所有的数都加到小跟堆里边去
			pQ.add(arr[i]);
		}
		int sum = 0;
		int cur = 0;
		while (pQ.size() > 1) {
			cur = pQ.poll() + pQ.poll();//相加
			sum += cur;
			pQ.add(cur);//将合完的结果仍进小跟堆里边去
		}
		return sum;
	}
