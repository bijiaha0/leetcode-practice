...
public class AbstractBinarySearchTree {
	...
	* 插入
	* */
	public Node insert(int element) {//最后生成的树，节点给我返回
		if (root == null) {
	...
	protected Node delete(Node deleteNode) {
		if (deleteNode != null) {
			...
			if (deleteNode != null) {
				if (deleteNode.left == null) {//如果删除的节点左边是空
					nodeToReturn = transplant(deleteNode, deleteNode.right);
				} else if (deleteNode.right == null) {//如果删除的节点右边是空
					nodeToReturn = transplant(deleteNode, deleteNode.left);
				} else {
					Node successorNode = getMinimum(deleteNode.right);//找到右子树最左的节点
